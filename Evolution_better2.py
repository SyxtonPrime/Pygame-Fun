...
def celltransitioner(size, grid, j, k, inc):
	...
	num = len(avInt)
	if num > 2:
		avInt.remove(max(avInt))
		avInt.remove(min(avInt))
		avStr.remove(max(avStr))
		avStr.remove(min(avStr))
		avAgi.remove(max(avAgi))
		avAgi.remove(min(avAgi))
		num -= 2
	avInt = sum(avInt)/num
	avStr = sum(avStr)/num
	avAgi = sum(avAgi)/num
	if (Int <= avInt) & (Str <= avStr) & (Agi <= avAgi): #Make == into <= to fix bias error
		num = randint(0, 5)
		if num == 0:
